...
def perform_test(test_loader, model, test_meter, cfg, writer=None):
    ...
    for cur_iter, (inputs, labels, video_idx, time, meta) in enumerate(
        ...
        if cfg.NUM_GPUS > 1:
            ...
            #     break
        # save the img_encode and labels
        torch.save(preds.cpu(), os.path.join(cfg.OUTPUT_DIR, 'preds', '%d.pth'%cur_iter))
        torch.save(labels.cpu(), os.path.join(cfg.OUTPUT_DIR, 'labels', '%d.pth'%cur_iter))
        logger.info("save img_encode and labels")
