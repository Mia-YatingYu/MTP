...
class TemporalClipVideo(nn.Module):
        ...
        if self.training:
            ...
            if self.keep_raw_model and (self.ensemble_pred or self.distillation):
                with torch.no_grad():
                    ...
                    #     max_t, _ = torch.max(sims, dim=1, keepdim=False)
                return [pred, img_encode, self.dynamic_classifier], [None, raw_img_encode, self.dynamic_classifier]
    ...
    def achieve_xcsf_matrix(self, text_feature_file):
        """feature matrix for spatiotemporal text augmentation"""
